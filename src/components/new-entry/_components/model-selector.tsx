...
import { IModel, ModelType } from '../_data/models';

interface IModelSelectorProps extends PopoverProps {
  types: readonly ModelType[];
  models: IModel[];
}

export function ModelSelector({ models, types, ...props }: IModelSelectorProps) {
  const [open, setOpen] = React.useState(false);
  const [selectedModel, setSelectedModel] = React.useState<IModel>(models[0]);
  const [peekedModel, setPeekedModel] = React.useState<IModel>(models[0]);
...
interface IModelItemProps {
  ...
  isSelected: boolean;
  onSelect: () => void;
  onPeek: (model: IModel) => void;
}

function ModelItem({ model, isSelected, onSelect, onPeek }: IModelItemProps) {
